...
namespace MFI
{
    ...
    public class ReportController: BaseController
    {
        ...
        [HttpGet("GetOpenings/{transaction_date}", Name="GetOpenings")]
        // [Authorize]
        public dynamic GetOpenings(int year, int month) {
          ...
          [HttpGet("GetTotalWoDates", Name="GetTotalWoDates")]
        // [Authorize]
        public dynamic GetTotalWoDates() {
            dynamic objresponse = null;

            decimal incOpening=0;
            decimal expOpening=0;

            var _incOpening =  _repositoryWrapper.Income.FindAll();
            foreach(var inc1 in _incOpening){
                incOpening +=inc1.Amount;
            }

            var _expOpening =  _repositoryWrapper.Expense.FindAll();
            foreach(var exp1 in _expOpening){
                expOpening +=exp1.Amount;
            }



            return objresponse = new {data = incOpening};
        }
