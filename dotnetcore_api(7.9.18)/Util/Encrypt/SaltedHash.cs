...
namespace Operational.Encrypt
{
    public class SaltedHash
    {
       ...
       /*     public static string ComputeHash1(string password)
        {
            //var saltBytes = Convert.FromBase64String(salt);
            using (var rfc2898DeriveBytes = new Rfc2898DeriveBytes(password,  PBKDF2_ITERATIONS))
                return Convert.ToBase64String(rfc2898DeriveBytes.GetBytes(HASH_BYTE_SIZE));
        }

        public static bool Verify1(string salt, string hash, string password)
        {
            return hash == ComputeHash1( password);
        } */
    }
}	